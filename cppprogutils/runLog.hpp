...
class runLog {
 public:
  ...
  /** Empty constructor, file won't be opened until setFilenameAndOpen() is called
   *
   */
  runLog(){
...
public:
  ...
  /**Open the runLog file with this name, and the start time will be now, will fail if runLog already opened
  ...
  /**Open the runLog file with this name and given start time, will fail if runLog already opened
   * @param fileName The filename for the runLog
  ...
  /** Put starting stamp on runLog by putting date and the command given for the program, will fail if runLog not started
   *
   * @param inputCommands The commands for the current program that contains the commandline for logging
   *
   */
  ...
  /** Log the current time difference from when the time point start_, will fail if runLog not started
   *
   */
...
/**Input operator to log information to the runLogFile_ for the given runLog, will fail if runLog not started
 *
 */
template<typename T>
